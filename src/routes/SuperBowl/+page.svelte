...
<section class="min-h-screen bg-slate-50 text-slate-900">
	<div class="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10">
		...
		<div class="flex justify-center">
			<button 
				class={`rounded-full px-8 py-3 text-sm font-semibold shadow-lg transition ${submissionsOpen ? 'bg-slate-900 text-white hover:bg-slate-800' : 'cursor-not-allowed bg-slate-200 text-slate-400'}`}
				on:click={() => {
					if (submissionsOpen) showForm = !showForm;
				}}
				disabled={!submissionsOpen}
